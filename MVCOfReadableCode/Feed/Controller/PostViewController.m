...
- (ViewEventsBlock)likeViewEventHandler{
    return ^(id<ViewEventsParam> param){
        NSLog(@"处理帖子点赞事件");
...
- (ViewEventsBlock)inputEmotionViewEventHandler{
    return ^(id<ViewEventsParam> param){
        NSLog(@"处理表情输入");
    };
}

- (ViewEventsBlock)showUserProfileEventHandler{
    return ^(id<ViewEventsParam> param){
...
- (ViewEventsBlock)commentMoreEventHandler{
    return ^(id<ViewEventsParam> param){
        NSLog(@"处理点击评论的更多");
    };
}
